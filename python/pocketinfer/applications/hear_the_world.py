...
class HearTheWorld(BaseApplication):
    ...
    # The translator is not wrong about these words; it picks the everyday
    # sense of a word the manual uses clinically, and the everyday sense
    # shares no stem with the page that holds the answer. "पानी की कमी"
    # arrived as "water scarcity" and "no water" - the manual says
    # dehydration on all four of those pages, so the question and its answer
    # had no word in common. Each pair was chosen by translating the real
    # question both ways and keeping the wording that produced the term the
    # index is actually built on:
    #
    #   पानी की कमी  water scarcity / no water -> dehydration   p26-30
    #   यौन रोग      sexual dysfunction        -> sexually       p47
    #   यौन संक्रमण   sexual transition         -> transmitted    p46,48
    #   दम घुटना     choking                   -> asphyxia       p54
    #   छल्ला        cut open                  -> IUCD           p43
    #   नसबंदी       vasectomy, for a woman    -> sterilisation  p44
    #   पसली चलना    walk rib                  -> chest          p31-32
    #   से बचाव      protect against           -> prevention     p61
    #
    # नसबंदी deliberately becomes the gender-neutral बंध्याकरण rather than
    # महिला नसबंदी, which translates more cleanly but would answer a question
    # about a man's operation from the pages about a woman's.
    _HI_FOR_TRANSLATION = tuple((re.compile(pat), rep) for pat, rep in (
        (r'पीप', 'मवाद'),
        (r'पानी\s+की\s+कमी', 'निर्जलीकरण'),
        (r'यौन\s+रोग', 'यौन संचारित संक्रमण'),
        (r'यौन\s+संक्रमण', 'यौन संचारित संक्रमण'),
        (r'दम\s*घुट\S*', 'श्वासावरोध'),
        (r'पसली\s*चल\S*', 'छाती का अंदर खिंचना'),
        (r'छल्ल[ाेो]', 'आई यू सी डी'),
        (r'नसबंदी', 'बंध्याकरण'),
        (r'से\s+बचाव', 'की रोकथाम'),
    ))

    # The translator reads a bare आशा as the noun it also is - hope - so
    # "टीबी के इलाज में आशा की क्या भूमिका है" arrives at the model as "what
    # is the role of hope in TB treatment", and a page that is entirely about
    # the worker no longer looks like an answer to the question being asked.
    # Two of the ten refused questions in the page-by-page test were this and
    # nothing else. Naming the role fixes it: over the six manual questions
    # that use the word, a bare आशा renders as "hope" in five, and
    # आशा कार्यकर्ता renders as ASHA in all six. Only a bare one is expanded,
    # and only where a Devanagari letter does not precede it, so निराशा and a
    # question that already says कार्यकर्ता, बहन or दीदी are left alone.
    _HI_ASHA = re.compile(
        r'(?<![\u0900-\u097F])आशा(?!\s*(?:कार्यकर्ता|कार्यकर्त्ता|बहन|बहिन|दीदी|जी))')

    @classmethod
    def _prepare_hindi_for_translation(cls, text):
        text = text or ''
        for weak, strong in cls._HI_FOR_TRANSLATION:
            text = weak.sub(strong, text)
        return cls._HI_ASHA.sub('आशा कार्यकर्ता', text)
    ...
    # The model does not always refuse in the words it was told to use. Asked
    # about a dog with a fever it answered "The context does not provide
    # information about a dog\'s fever", which is a refusal, but not one the
    # sentinel catches - so the device translated it and read out the word
    # "context" to an ASHA who has never been shown one. Anything that refuses
    # by talking about the passage instead of the subject is turned back into
    # the fixed refusal she is meant to hear. Checked against all fifty
    # answered questions of the page-by-page set: none of them match.
    _META_REFUSAL = re.compile(
        r"\b(?:context|manual|passage|document)\b[^.]{0,40}?"
        r"\b(?:does not|doesn't|do not|don't|did not)\b[^.]{0,30}?"
        r"\b(?:provide|contain|mention|include|specify|state|say|have)\b"
        r"|\bno information (?:about|on|regarding)\b"
        r"|\bnot (?:mentioned|provided|specified|stated|found) (?:in|within) the\b",
        re.I)

    @classmethod
    def _is_no_answer(cls, text):
        """True when the model declined, in the words it was asked to use or
        in its own."""
        text = text or ''
        return (cls.NO_ANSWER_SENTINEL in text.upper()
                or bool(cls._META_REFUSAL.search(text)))
