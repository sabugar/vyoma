...
class ShapedTextBox:
    '''A text area rendered through Pillow/HarfBuzz instead of displayio labels.

    adafruit_display_text blits one bitmap glyph per code point, left to right,
    with no shaping. That cannot render Devanagari: matras never attach and
    float above the line, the pre-base i-matra is not reordered ("मासिक"
    displayed as "मासक"), and conjuncts never form ("स्तनपान" displayed as
    "सतनपान"). Latin came out wobbly too, since the .pcf was built for the
    Devanagari metrics.

    Pillow here is built with raqm/HarfBuzz, so shaping the string into a
    1-bit image and pushing that into a displayio.Bitmap renders both scripts
    correctly. Measured ~33ms per update (17ms shape/draw + 16ms blit), which
    is well inside the budget for a screen that changes a few times per query.
    '''

    def __init__(self, x, y, width, height, font_path, font_size, color,
                 line_spacing=2):
        self.width = width
        self.height = height
        ...
        self.tilegrid = displayio.TileGrid(
            self.bitmap, pixel_shader=self.palette, x=x, y=y)
        self._text = ""

    def _wrap(self, text):
        '''Greedy word wrap using shaped widths, so the measurement matches
        what is actually drawn (Devanagari clusters are narrower than the sum
        ...
        for paragraph in text.split("\n"):
            ...
            for word in words:
                ...
                if self.font.getlength(candidate) <= self.width - 4:
                    line = candidate
                else:
                    if line:
                        lines.append(line)
    ...
    def text(self, value):
        ...
        for line in self._wrap(self._text):
            if y + self.line_height > self.height:
                break  # silently clip rather than overflow into the next area
            draw.text((2, y), line, font=self.font, fill=1, language="hi")
            y += self.line_height
        pixels = img.load()
        bitmap = self.bitmap
...
class HandheldUI:
    ...
    # Outline font for the shaped (Devanagari-capable) text areas. Shipped by
    # fonts-deva-extra / lohit-devanagari on the device image.
    TTF_FONT_PATH = '/usr/share/fonts/truetype/lohit-devanagari/Lohit-Devanagari.ttf'
    ...
    def __init__(self, display, touch, logger=None):
        ...
        # Question and answer areas carry Devanagari, which displayio's label
        # rendering cannot shape - see ShapedTextBox for the details.
        self.toptext = ShapedTextBox(
            x=0, y=16, width=320, height=100,
            font_path=self.TTF_FONT_PATH, font_size=15, color=color)
        self.appui.append(self.toptext.tilegrid)

        self.bottomtext = ShapedTextBox(
            x=0, y=118, width=320, height=100,
            font_path=self.TTF_FONT_PATH, font_size=15, color=color)
        self.appui.append(self.bottomtext.tilegrid)
    ...
    The proxy object will have the same methods as the UI class, and will send requests to the UI process and wait for the result.  '''
        class RemoteUI:
            def __init__(self, rpc_pipe):
                ...
                # hear_the_world.py calls board.top_text()/bottom_text()/etc. from
                # both the main loop thread and its delayed_write_* daemon threads
                # concurrently. multiprocessing.connection.Connection.send()/recv()
                # are not safe to call from multiple threads at once - concurrent
                # calls can interleave mid-message and corrupt the framing (seen
                # live as `_pickle.UnpicklingError: invalid load key`). Serialize
                # all access to this one shared pipe with a lock.
                self._lock = threading.Lock()

            def __getattr__(self, name):
                def remote_call(*args):
                    # UIRPCCall.execute() (run in the UI subprocess) always sends
                    # a response back after a call runs, but this proxy is meant
                    # to be fire-and-forget (callers don't use the return value).
                    # Drain any responses already sitting in the pipe before
                    # sending the next call - otherwise unread responses pile up
                    # in the OS pipe buffer over the life of the process until it
                    # fills and every future send() blocks forever, silently
                    # freezing the main interaction loop.
                    with self._lock:
                        while self.rpc_pipe.poll(0):
                            try:
                                self.rpc_pipe.recv()
                            except EOFError:
                                break
